...
public class DuelManager implements Listener {
	...
	public void on(PlayerDeathEvent event) {
		...
		if (!arena.isEmpty()) {
			...
			Optional<UserData> optPlayer = dataManager.getUser(dead.getUniqueId());
			Optional<UserData> optOpponent = dataManager.getUser(target.getUniqueId());

			// Add if both are not null
			if (optPlayer.isPresent() && optOpponent.isPresent()) {
				UserData player = optPlayer.get();
				UserData opponent = optOpponent.get();
